...
- งานที่ประเมินว่า > 20 นาที — พิจารณา parallel: **Layer cake** (แบม + น้องชีส + น้องวิว เขียนคนละไฟล์), **Multi-page sweep** (ใช้ Workflow tool `parallel()`), **Read-only review** (ติ๊ก + แป้ง + ทีมวิเคราะห์ อ่านขนาน). รายละเอียด + anti-pattern ที่ [[parallel-dispatch-patterns]]
- ห้าม parallel: migration apply, Edge Function deploy, แก้ไฟล์เดียวกัน 2 ตัว (เว้นใช้ `isolation: "worktree"`)

## ทีมวิเคราะห์ + รายงาน (เพิ่ม 2026-07-27)

4 คนใหม่ **อ่านอย่างเดียว วิเคราะห์+เสนอ ห้ามแตะโค้ด**: **โบว์** (การตลาด/โปรโมชั่น) · **นุ่น** (วิเคราะห์ข้อมูล) · **ตาล** (การเงิน) · **มุก** (เขียนรายงาน)

- ทั้ง 4 คน **ไม่มีสิทธิ์ต่อ DB** — เขียน SQL ส่งให้ครีมรันผ่าน Supabase MCP แล้วส่งผลกลับ. **SELECT เท่านั้น** ห้าม mutate เด็ดขาด
- คำถามที่ขึ้นต้นด้วยตัวเลข → **นุ่นก่อนเสมอ** (ล็อกนิยาม metric ก่อน ไม่งั้นโบว์/ตาลเถียงกันคนละตัวหาร)
- **มุกเป็นคนสุดท้ายเสมอ** — ต้องได้ตัวเลขพร้อมที่มาจากนุ่น/ตาล/โบว์ ถึงจะเขียนรายงานได้
- ผลวิเคราะห์ที่ควรกลายเป็นหน้าในเว็บ → analyst เขียน spec → ครีมส่งน้องวิว/น้องชีสทำต่อ (กฎ 0 เหมือนเดิม)
