-- 0044: Seed 29 partner shops + เพิ่ม shop_code lookup ladder ใน import_pj_batch
-- ============================================================================
-- Part A: seed ร้านพาร์ทเนอร์ 29 ร้านที่หายไป (idempotent — WHERE NOT EXISTS)
--   หมายเหตุ: shops.code ไม่มี unique constraint (ดู 0001_init.sql บรรทัด 52-61)
--   จึงใช้ WHERE NOT EXISTS per row แทน ON CONFLICT
--
-- Part B: ขยาย import_pj_batch RPC ให้ lookup shop ด้วย shop_code ก่อน
--   ladder ใหม่:
--     1) ถ้า JSON มี shop_code (non-empty) → match shops.code ตรงๆ
--     2) ถ้ายังหาไม่เจอ → match lower(trim(name)) เหมือนเดิม
--     3) ถ้ายังหาไม่เจอ + p_create_new_shops → สร้างใหม่
--        - ใช้ shop_code จาก JSON ถ้ามี
--        - ถ้าไม่มี → auto-generate 'PJ-...' เหมือนเดิม
--
--   ⚠️ ส่วน body อื่นเหมือน 0043 ทุกอย่าง (term_months=0 workaround,
--      parse_pj_address, payment_log, ฟิลด์ใหม่ 0043, idempotency, EXCEPTION)
-- ============================================================================

-- ============================================================================
-- Part A: seed 29 ร้านพาร์ทเนอร์
-- ============================================================================

insert into public.shops (code, name, active)
select v.code, v.name, v.active
from (values
  ('JA02',      N'ร้านจ่าอั๋นโฟน สาขา 2',         true),
  ('JA04',      N'ร้านจ่าอั๋นโฟน สาขา 4',         true),
  ('AQ S00001', N'ร้านโอ่ง ช็อป',                  true),
  ('AQ S00003', N'ร้านพลอยโมบาย โรบิสันกาญจบุรี', true),
  ('AQ S00004', N'ร้านแนทโฟน',                    true),
  ('AQ S00005', N'ร้านLinermobile',                true),
  ('AQ S00006', N'ร้านBFz Phone',                  true),
  ('AQ S00007', N'ร้านน้ำหวาน สาขากรุงเทพ',       true),
  ('AQ S00008', N'ร้านL.A.mobile',                 true),
  ('AQ S00009', N'ร้านน้ำหวาน สาขาสกลนคร',        true),
  ('AQ S00010', N'ร้านน้ำหวาน สาขาอุดร',          true),
  ('AQ S00011', N'ร้านJNPhone',                    true),
  ('AQ S00012', N'ร้าน ID Mobile',                 true),
  ('AQ S00013', N'A.M.Mobile',                     true),
  ('AQ S00014', N'ร้านเปิ้ลโฟน',                   true),
  ('AQ S00015', N'ร้านนะโมโอเคโฟน',                true),
  ('AQ S00018', N'ร้านคณาโฟนรังสิต',               true),
  ('AQ S00019', N'ร้านลลินโมบาย ขอนแก่น',          true),
  ('AQ S00020', N'ร้าน เฟิร์นดีโฟน',               true),
  ('AQ S00021', N'ร้านทูบีโมบายโฟน',               true),
  ('AQ S00022', N'ร้านกัสเกรซโมบาย',               true),
  ('AQ S00023', N'ร้านตาต้าโมบาย',                 true),
  ('AQ S00024', N'ร้านพลอยไอโฟนช็อป',              true),
  ('AQ S00025', N'ร้านวินนี่โฟน',                  true),
  ('AQ S00026', N'ร้าน ICloud station',            true),
  ('AQ S00027', N'ร้านPO Phone',                   true),
  ('AQ S00028', N'ร้าน LiuPhone',                  true),
  ('AQ S00029', N'ร้าน ยินดีโมบาย',                true),
  ('AQ S00030', N'A&K Mobile',                     true)
) as v(code, name, active)
where not exists (
  select 1 from public.shops s where s.code = v.code
);

-- ============================================================================
-- Part B: import_pj_batch — เพิ่ม shop_code lookup ladder
-- ============================================================================
...
-- 1) ตรวจ seed:
--    SELECT count(*) FROM public.shops WHERE active = true;
--    -- expected: 31 (2 เดิม + 29 ใหม่)
--    SELECT code, name FROM public.shops
--     WHERE code IN ('JA02','JA04','AQ S00001','AQ S00030') ORDER BY code;
--
-- 2) Idempotency: รัน 0044 ซ้ำต้องไม่ insert duplicate
--    (count คงเดิม 31)
--
-- 3) ตรวจ function signature ไม่เปลี่ยน:
...
--
-- 4) Dry-run shop_code lookup (BEGIN/ROLLBACK):
--    BEGIN;
--      -- ใช้ shop_code='JA02' ที่เพิ่ง seed → ต้องเจอโดยไม่สร้างร้านใหม่
--      SELECT public.import_pj_batch(
--        '[{"invoice_no":"INV-T-0044","shop_code":"JA02",
--           "shop_name":"ชื่อมั่ว ไม่ตรง DB",
--           "customer_name":"ลูกค้าทดสอบ","trade_date":"2025-01-15",
--           "finance_amount":"15000","down_payment":"5000",
--           "monthly_payment":"1000","first_due_date":"2025-02-15"}]'::jsonb,
--        '[{"invoice_no":"INV-T-0044","row_no":"1","payment_type":"ค่างวด",
--           "amount":"1000","due_date":"2025-02-15","status":"pending"}]'::jsonb,
--        44, false
--      );
--      SELECT c.contract_no, s.code, s.name
--        FROM contracts c JOIN shops s ON s.id = c.shop_id
--       WHERE c.contract_no='INV-T-0044';
--      -- expected: shop.code='JA02' (lookup โดย code ไม่ใช่ชื่อมั่ว)
--    ROLLBACK;
