...
const ALLOWED_ROLES = ["admin", "staff", "freelancer", "executive", "accounting"] as readonly string[];
  ...
  fetch: withSupabase({ auth: ["publishable", "secret"] }, async (req, _ctx) => {
    ...
    try {
      ...
      if (action === "create") {
        ...
        if (!ALLOWED_ROLES.includes(role)) return json({ error: "role ต้องเป็น admin, staff, freelancer, executive หรือ accounting" }, 400);
      ...
      if (action === "update") {
        ...
        if (typeof role === "string" && ALLOWED_ROLES.includes(role)) patch.role = role;
