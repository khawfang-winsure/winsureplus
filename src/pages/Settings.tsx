...
import { Briefcase, Coins, Pencil, Plus, ShieldCheck, Smartphone, Store, Tag, Trash2, type LucideIcon } from 'lucide-react'
...
type CategoryKey = 'shops' | 'device' | 'job' | 'promo' | 'commission' | 'perm'
const CATEGORIES: { key: CategoryKey; label: string; icon: LucideIcon; kinds: OptionKind[] }[] = [
  ...
  { key: 'commission', label: 'ค่าคอมมิชชั่น', icon: Coins, kinds: [] },
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-col gap-4">
            ...
            {cat === 'commission' && <CommissionEditor canEdit={canEdit} />}
            {cat !== 'shops' &&
              cat !== 'perm' &&
              cat !== 'commission' &&
...
// ===== ตัวแก้ขั้นบันไดค่าคอมมิชชั่น (ตั้งค่าได้) =====
function CommissionEditor({ canEdit }: { canEdit: boolean }) {
  ...
  return (
    <Card>
      <div className="mb-1 flex items-center gap-2">
        ...
        <h3 className="font-semibold text-ink">ขั้นบันไดค่าคอมมิชชั่น</h3>
      </div>
      <p className="mb-4 text-sm text-ink-soft">
        หน่วย <span className="font-semibold text-ink">บาท/เคส</span> · คิดตอนบันทึกสัญญา · ยิ่งทำเยอะ เรตยิ่งสูงได้
      </p>
      ...
      <p className="mt-4 rounded-lg bg-peach-light/40 px-3 py-2 text-xs text-ink-soft">
        ขั้นถัดไป: ระบบจะนับจำนวนเคสที่แต่ละคน “บันทึก” แล้วคำนวณค่าคอมตามขั้นนี้ + หักคืนเมื่อลูกค้าไม่จ่ายงวดแรก
        (ล่าช้า&gt;30วัน) หรือคืนเครื่อง
      </p>
    </Card>
  )
}
