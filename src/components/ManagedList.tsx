...
import { useListControls, type ListControls } from '../lib/useListControls'
...
// แถบเครื่องมือ: ค้นหา / เรียงตามชื่อ / จำนวนต่อหน้า — ใช้กับลิสต์หรือตารางก็ได้
...
// แถบแบ่งหน้า (สรุปจำนวน + ปุ่มก่อนหน้า/ถัดไป)
export function ListPager({ controls: c }: { controls: ListControls }) {
  if (c.total === 0) return null
  return (
    <div className="mt-3 flex items-center justify-between text-sm text-ink-soft">
      <span>
        ทั้งหมด {c.total} รายการ{c.pageCount > 1 ? ` · หน้า ${c.page}/${c.pageCount}` : ''}
      </span>
      {c.pageCount > 1 && (
...
// ลิสต์แบบแถว ที่มี ค้นหา + เรียง + แบ่งหน้า ครบในตัว
export function ManagedList<T extends { id: string }>({
  ...
  return (
    <div>
      <ListToolbar controls={c} searchPlaceholder={searchPlaceholder} right={toolbarRight} />
      {c.total === 0 ? (
